...
在DevOps中开发扮演的角色仍然是最关键的。但一般意义上一个系统的整个流程仍需要运维的大量参与。而我所理解的或者说我认为更好的一种方式就是开发能够兼职运维的角色，承担一部分部署、故障监控责任，这样能够更好地保证系统的部署、发布以及后续的线上维护。所以，在这一点上开发需要具有运维的一部分技能，才能够承担一部分运维的责任。
...
* 硬件配置：这个无需多说
* 数据库设置：max\_connection的一些配置会影响数据库的连接数
...
通过使用以下十条命令，可以在1分钟内对系统资源使用情况有个大致的了解。
